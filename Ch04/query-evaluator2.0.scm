...
(define (conjoin conjuncts frame-stream)
  (define (unify-vals frame1 frame2)
    (cond ((stream-null? frame1) frame2)
          ((binding-in-frame (binding-variable (car frame1)) frame2)
           (if (equal? (binding-value (binding-in-frame (binding-variable (car frame1)) frame2)) (binding-value (car frame1)))
               (unify-vals (cdr frame1) frame2)
               #f))
          (else (unify-vals (cdr frame1) (extend (binding-variable (car frame1)) (binding-value (car frame1)) frame2)))))
  (define (make-ans frame-list1 frame-list2)
    (define (loop frames)
      (define (internal-loop frames2)
        (if (stream-null? frames2)
            (loop (stream-cdr frames))
            (let ((attempt (unify-vals (stream-car frames) (stream-car frames2))))
              (if attempt
                  (cons-stream attempt (internal-loop (stream-cdr frames2)))
                  (internal-loop (stream-cdr frames2))))))
      (if (stream-null? frames)
          the-empty-stream
          (internal-loop frame-list2)))
    (loop frame-list1))
  (if (null? conjuncts)
      frame-stream
      (make-ans (qeval (first-conjunct conjuncts) frame-stream) (conjoin (rest-conjuncts conjuncts) frame-stream))))
...
(define (unique-contents exp) (car exp))

(define (unique exp frame-stream)
  (define (check-if-valid frame-stream)
    (and (not (stream-null? frame-stream))
         (stream-null? (stream-cdr frame-stream))))
  (stream-flatmap
    (lambda (frame)
      (let ((queries (simple-query (unique-contents exp) (singleton-stream frame))))
    (if (check-if-valid queries) queries the-empty-stream)))
    frame-stream))
